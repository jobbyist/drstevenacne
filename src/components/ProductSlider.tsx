          ...
          <div className="overflow-hidden -mx-4" ref={emblaRef}>
            <div className="flex">
              {products.map((p) => (
                <div
                  key={p.node.id}
                  className="pl-4 min-w-0 basis-[78%] sm:basis-[48%] lg:basis-[32%] xl:basis-[26%]"
                >
...
function ProductCard({ product }: { product: ShopifyProduct }) {
  ...
  return (
    <Link to="/product/$handle" params={{ handle: product.node.handle }} className="group block">
      ...
      <div className="pt-5">
        <div className="text-[11px] uppercase tracking-[0.18em] text-clinical font-semibold">
          {product.node.productType}
        </div>
        <h3 className="mt-1.5 text-lg font-medium text-primary group-hover:text-clinical transition">
          {product.node.title}
        </h3>
